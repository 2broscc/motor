...
if ($d>0)
	...
	$t->assign("FORUMS_POSTS_PAGEPREV",
		"<a href=\"forums.php?m=posts&amp;q=$q&amp;d=$prevpage\">".$L['Previous']." $sed_img_left</a>");
	}
...
if (($d + $cfg['maxtopicsperpage'])<$totalposts)
	...
	$t->assign("FORUMS_POSTS_PAGENEXT",
		"<a href=\"forums.php?m=posts&amp;q=$q&amp;d=$nextpage\">$sed_img_right ".$L['Next']."</a>");
	$notlastpage = TRUE;
	}
...
$toptitle .= " ".$cfg['separator']." <a href=\"forums.php?m=posts&amp;q=".$q."\">".sed_cc($ft_title)."</a>";
...
if (!$notlastpage && !$ft_state && $usr['id']>0 && $allowreplybox && $usr['auth_write'])
	...
	if ($quote>0)
		{
		$sql4 = sed_sql_query("SELECT fp_text, fp_postername, fp_posterid FROM $db_forum_posts WHERE fp_topicid='$q' AND fp_sectionid='$s' AND fp_id='$quote' LIMIT 1");
