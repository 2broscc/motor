...
if ($usr['isadmin'] && !empty($q) && !empty($a))
	...
	switch($a)
		...
		if ($ghost)
			{
			$sql1 = sed_sql_query("SELECT ft_title, ft_mode, ft_creationdate, ft_firstposterid, ft_firstpostername FROM $db_forum_topics WHERE ft_id='$q' and ft_sectionid='$s'");
			}
...
$jumpbox = "<form id=\"jump\"><select name=\"jumpbox\" size=\"1\" onchange=\"redirect(this)\">";
...
if ($d>0)
	...
	$t->assign("FORUMS_TOPICS_PAGEPREV",
		"<a href=\"forums.php?m=topics&amp;s=$s&amp;d=$prevpage&amp;o=$o&amp;w=$w\">".$L['Previous']." $sed_img_left</a>");
	}
...
if (($d + $cfg['maxtopicsperpage'])<$totaltopics)
	...
	$t->assign("FORUMS_TOPICS_PAGENEXT",
		"<a href=\"forums.php?m=topics&amp;s=$s&amp;d=$nextpage&amp;o=$o&amp;w=$w\">$sed_img_right ".$L['Next']."</a>");
	}
...
for ($i = 0; $i < $nbpages; $i++)
	...
	if ($i==$curpage)
		{ $pages .= "&gt; <a href=\"forums.php?m=topics&amp;s=$s&amp;d=$j&amp;o=$o&amp;w=$w\">".($i+1)."</a> &lt; "; }
