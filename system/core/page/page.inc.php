...
if (!empty($pag['page_extra3']) and (!empty($pag['page_extra4']))) 
	...
	//print"Foglalt:page_extra3 és page_extra4";
...
//similar pages query
$similar_limit = 8;
$similar_sql = mysql_query("SELECT  * FROM $db_pages ORDER BY 'page_date' DESC LIMIT 8");
	while( $row = mysql_fetch_array($similar_sql) ) {
	
 				
	$s_mask .= "".$row['page_cat']."//<a href=\"page.php?id=".$row['page_id']."\"><h4>".$row['page_title']."</h4></a>";
...
$mskin = "$user_skin_dir/".$skin."/page.tpl"; 
...
//facebook comment
$url_id = $pag['page_id'];
$page_fb_comment = "<fb:comments width=\"770\" xid=\"$fb_comment_id\" url=\"http://www.ridelinemtb.hu/page.php?id=$fb_comment_id\" canpost=\"true\" candelete=\"false\" numposts=\"14\" publish_feed=\"true\"></fb:comments>";


//sed_facebook_comment_api();
...
$t->assign(array(
	...
	"PAGE_PRINT" => "<a href=\"page.php?id=$print_page&print=page\"><img title=\"Oldal nyomtatható változata\" src=\"skins/".$cfg['defaultskin']."/img/icons/page_print.png\"/></a>",
	"PAGE_EMAIL_ME" => "<a href=\"javascript:mailpage()\"><img title=\"Oldal e-mail-ben való küldése\" src=\"skins/".$cfg['defaultskin']."/img/icons/page_email_link.png\"/></a>",
...
//for meta tags
	...
	"PAGE_IMG_META" => $pag['page_desc'],
	
	
	"PAGE_DESC" => "<img src=\"".$pag['page_desc']."\" width=\"114\" heigth=\"84\"  alt=\"Nincs kép megadva!\" />",
//	"PAGE_AUTHOR" => $pag['page_author'],
	...
	"PAGE_UDEF_SIMILAR_LINKS" => $s_mask,
	"PAGE_UDEF_FACEBOOK_COMMENTS" => sed_facebook_comment($pag['page_id'], "http://mbedsolutions.atw.hu/page.php?id=".$pag['page_id']."","10","594px"),
