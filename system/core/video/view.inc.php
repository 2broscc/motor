...
if (eregi("\.", $v) || eregi("/", $v)) { die('Wrong URL.'); }
...
if (preg_match('@<head>(.*?)</head>@si', $vd, $ext_head)==1) { $ext_head = $ext_head[1]; }

if (preg_match('@<body[^>]*?>(.*?)</body>@si', $vd, $ext_body)==1) { $ext_body = $ext_body[1]; }
...
$t = new XTemplate("$user_skin_dir/".$skin."/plugin.tpl");
