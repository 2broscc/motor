...
if($cfg['plugin']['tags']['pages'])
{
	...
	foreach($tcloud as $tag => $cnt)
	{
		...
		//$tc_html .= '<a href="'.sed_url('plug', 'e=tags&a='.$cfg['plugin']['tags']['index'].'&t='.$tag_u.$tl).'" class="'.$dim.'">'.htmlspecialchars($tag_t).'</a> ';
	
		$tc_html .= "<a href=\"plug.php?e=tags&a=".$cfg['plugin']['tags']['index']."&t=".$tag_u.$tl."\" class=\"".$dim."\">".htmlspecialchars($tag_t)."</a>";
	
	}
	if($cfg['plugin']['tags']['more'] && $limit > 0)
	{
		//$tc_html .= '<hr /><a class="more" href="'.sed_url('plug', 'e=tags&a='.$cfg['plugin']['tags']['index']).'">'.$L['tags_All'].'</a>';
		$tc_html .= '<hr /><a class="more" href=\"plug?e=tags&a='.$cfg['plugin']['tags']['index'].'">'.$L['tags_All'].'</a>';
	}
	$tc_html .= '</div>';
