 ...
 * @version 0.7.0
 * @author sedonti Team
 * @copyright Copyright (c) sedonti Team 2008-2012
 * @license BSD
 */

defined('SED_CODE') or die('Wrong URL');

require_once sed_incfile('tags', 'plug', 'config');
require_once sed_langfile('tags', 'plug');
require_once sed_incfile('tags', 'plug', 'resources');
 ...
 * @return bool
 */
function sed_tag($tag, $item, $area = 'pages', $extra = null)
{
	global $db, $db_tag_references;
	$item = (int) $item;
	if (sed_tag_isset($tag, $item, $area, $extra))
	{
		return false;
	}
	$data = array(
...
function sed_tag_cloud($area = 'all', $order = 'tag', $limit = null)
{
	...
	$sql->closeCursor();
	$cache && $cache->db->store($cache_name, $res, sed_DEFAULT_REALM, 300);
	return $res;
}
 ...
 * @return int
 */
function sed_tag_count($tag, $area = '', $extra = null)
{
	global $db, $db_tag_references;
 ...
 * @return bool
 */
function sed_tag_isset($tag, $item, $area = 'pages', $extra = null)
{
	global $db, $db_tag_references;
	$item = (int) $item;
 ...
 * @return array
 */
function sed_tag_list($item, $area = 'pages', $extra = null)
{
	global $db, $db_tag_references;
	$res = array();
 ...
 * @return bool
 */
function sed_tag_remove($tag, $item, $area = 'pages', $extra = null)
{
	global $db, $db_tag_references;
	if (sed_tag_isset($tag, $item, $area, $extra))
	{
 ...
 * @return int
 */
function sed_tag_remove_all($item = 0, $area = 'pages', $extra = null)
{
	global $db, $db_tag_references;
	if ($item == 0)
	{
...
function sed_tag_search_form($area = 'all')
{
	...
	foreach ($tcloud as $tag => $cnt)
	{
		...
		$tag_u = $cfg['plugin']['tags']['translit'] ? sed_translit_encode($tag) : $tag;
		...
		$tc_html .= sed_rc('tags_link_cloud_tag', array(
			'url' => sed_url('plug', array('e' => 'tags', 'a' => $area, 't' => $tag_u, 'tl' => $tl)),
	...
	if ($perpage > 0)
	{
		...
		$pagenav = sed_pagenav('plug','e=tags&a=' . $area, $dt, $totalitems, $perpage, 'dt');
		$t->assign(array(
